#!/usr/bin/env python3
"""
YouTuber's Stock Scanner with SL, T1, T2 - FIXED VERSION
=========================================================
Original YouTuber logic + Risk Management (CORRECTED)
"""
...
def advanced_stock_scanner(ticker_list):
    ...
    for ticker in ticker_list:
        try:
            ...
            # ============================================================
            # FIXED SL/T1/T2 CALCULATION
            # ============================================================
            
            # Stop Loss: Use ONLY 10-day swing low (not min with 30-DMA)
            # This ensures we always have valid SL below entry
            swing_low = data['Low'].tail(10).min()
            sl = swing_low  # ← FIXED: Just use swing low, don't min with DMA
            
            entry = cmp
            risk = entry - sl
            
            # Skip only if SL is at or above entry (should rarely happen)
            if risk <= 0:
                continue
...
if __name__ == "__main__":
    ...
    print("\n" + "="*120)
    print("🟢 RESULTS: Positive Breakout Stocks with SL, T1, T2 (FIXED)")
    print("="*120 + "\n")

    if result.empty:
        print("❌ No stocks found today.\n")
    else:
        display_cols = ['Stock', 'CMP', '30 DMA', '50 DMA', '200 DMA', 'SL', 'T1', 'T2', 'Reward %', 'R:R Ratio']
        print(result[display_cols].to_string(index=False))
        print("\n" + "="*120)
        print(f"Total: {len(result)} stocks | Avg Reward: {result['Reward %'].mean():.2f}% | Avg R:R: {result['R:R Ratio'].mean():.2f}:1")
        
        result.to_excel("YouTuber_Scanner_FIXED.xlsx", index=False)
        print("✅ Saved to 'YouTuber_Scanner_FIXED.xlsx'")
