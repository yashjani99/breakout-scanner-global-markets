#!/usr/bin/env python3
"""
YouTuber's Stock Scanner with SL, T1, T2 - FINAL CORRECTED VERSION
===================================================================
Fixed: Use .item() to extract scalar from Series
"""
...
def advanced_stock_scanner(ticker_list):
    ...
    for ticker in ticker_list:
        try:
            ...
            # Calculate Moving Averages - use float() to ensure scalar
...
if __name__ == "__main__":
    ...
    print("=" * 120)
    print("🟢 YouTuber Stock Scanner with SL, T1, T2 - FINAL CORRECTED")
    print("=" * 120 + "\n")
    ...
    if result.empty:
        ...
        display_cols = ['Stock', 'CMP', '30 DMA', '50 DMA', '200 DMA', 'SL', 'SL %', 'T1', 'T2', 'Reward %', 'R:R Ratio']
        print(result[display_cols].to_string(index=False))

        print("\n" + "=" * 120)
        print(f"✅ Total Stocks: {len(result)}")
        print(f"📊 Avg SL %: {result['SL %'].mean():.2f}% | Avg Reward: {result['Reward %'].mean():.2f}% | Avg R:R: {result['R:R Ratio'].mean():.2f}:1")
        print("=" * 120)

        result.to_excel("YouTuber_Scanner_WITH_SL_T1_T2.xlsx", index=False)
        print("✅ Saved: YouTuber_Scanner_WITH_SL_T1_T2.xlsx\n")
